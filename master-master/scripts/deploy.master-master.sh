...
master-master() {
    ...
    export USER_MONITOR_USERNAME=${USER_MONITOR_USERNAME:-'proxy'}
    export USER_MONITOR_PASSWORD=${USER_MONITOR_PASSWORD:-'12345678910'}
    ...
    docker exec $(docker ps -q -f name=$HOST_MASTER) \
        ...
        GRANT SELECT on *.* TO '$USER_MONITOR_USERNAME'@'%';\
